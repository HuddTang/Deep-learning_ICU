...
df_ = df = pd.read_csv('./data/train_LCCS_4.csv', encoding='utf-8')
...
label.to_csv('./data/train_LCCS_4_label.csv', encoding='utf-8', index=False)
...
print(df.info())
print(df)
df__ = pd.read_csv('./data/train_LCCS_onehot.csv', encoding='utf-8')
all = df__.columns.values[1:]
current = df.columns.values[1:]
final_df = df['ID']
for c in all:
    if c in current:
        final_df = pd.concat([final_df,df[c]], axis = 1)
    else:
        temp = pd.DataFrame({c: [0 for i in range(1182)]})
        final_df = pd.concat([final_df,temp], axis = 1)

print(final_df.info())
print(final_df)
final_df.to_csv('./data/train_LCCS_4_onehot.csv', encoding='utf-8', index=False)
